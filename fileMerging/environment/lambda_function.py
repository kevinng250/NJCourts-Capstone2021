...
def main(path,fileNames,docxTempFiles):
...
def lambda_handler(event=None, context=None):
    ...
    #changes with the new input format -- 4/15/2021, only the object name inside files array
    try:
        path = event['files'][0]
    ...
    for i in range(length):
        print('InputKey = ' , event['files'][i])
        #InputKeys.append(event['files'][i]['key'])
        InputKeys.append(event['files'][i])
    ...
    try:
        ...
        main(path,fileNames,docxTempFiles)
